...
export class TOTPAuthGuard implements CanActivate {
  ...
  async canActivate(context: ExecutionContext): Promise<boolean> {
    ...
    // Verify TOTP
    const action = this.reflector.get<string>('totp-action', context.getHandler()) || 'general';
    const isValid = await this.authService.verifyTOTP(user.userId, totpCode, action);
