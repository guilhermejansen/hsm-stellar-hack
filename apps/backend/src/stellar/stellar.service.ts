...
import { StellarTransactionRequest, StellarTransactionResult } from '../common/interfaces';
...
export class StellarService implements OnModuleInit {
  ...
  private async initializeStellarServices(): Promise<void> {
    try {
      ...
      const rpcUrl = this.configService.get('STELLAR_RPC_URL', 'https://soroban-testnet.stellar.org:443');
      ...
      const contractAddress = this.configService.get('STELLAR_CONTRACT_ADDRESS');
      if (contractAddress) {
  ...
  private async testConnection(): Promise<void> {
    try {
      ...
      this.logger.log(`🌐 Horizon connected - Latest ledger: ${ledger.records[0].sequence}`);
  ...
  async getAccountInfo(publicKey: string) {
    try {
      ...
      this.logger.error(`❌ Failed to get account info for ${publicKey}:`, error.message);
      throw error;
    }
  }
  ...
  async buildTransaction(request: StellarTransactionRequest): Promise<string> {
    try {
      this.logger.log(`🔨 Building transaction: ${request.amount} XLM to ${request.destinationAddress}`);

      // Load source account
      const sourceAccount = await this.horizonServer.loadAccount(request.sourceAddress);
  ...
  async submitTransaction(signedTransactionXDR: string): Promise<StellarTransactionResult> {
    try {
      ...
      const result = await this.horizonServer.submitTransaction(
        TransactionBuilder.fromXDR(signedTransactionXDR, this.networkPassphrase)
      );
  ...
  async getContractInfo() {
    try {
      ...
      return {
        ...
        version: '1.0.0',
        description: 'Stellar Custody Multi-Sig Contract (2-of-3) with HSM integration',
        isInitialized: true,
        guardianCount: 3,
        thresholds: {
          lowValue: 2,
  ...
  async initializeContract(
    ...
    try {
      ...
      this.logger.log(`   Guardians: ${guardians.map(g => `${g.role}:${g.address}`).join(', ')}`);
  ...
  async callContract(
    ...
    try {
      ...
      this.logger.error(`❌ Contract call failed (${functionName}):`, error.message);
      throw error;
    }
  }
