...
export class TransactionController {
  ...
  @ApiOperation({
    ...
    description: 'Create a new multi-sig transaction with automatic threshold determination'
  })
  ...
  @ApiOperation({
    ...
    description: 'Guardian approves transaction using challenge-response or TOTP'
  })
    ...
    try {
      ...
      if (result.executionReady) {
        ...
        if (transaction && transaction.stellarHash) {
          ...
          this.logger.log(`🎉 Stellar success stickers sent to ${guardians.length} guardians`);
        }
      }
        ...
        data: {
          ...
          privacyProtection: transaction.TransactionKey ? {
            ...
            keyStatus: {
              ...
              destroyedAt: transaction.TransactionKey.destroyedAt?.toISOString(),
  ...
  async getTransactionPrivacyReport(@Request() req: any) {
    try {
      const privacyReport = await this.transactionService.getTransactionPrivacyReport(req.user.userId);
