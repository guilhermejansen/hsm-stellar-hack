...
export class WalletController {
  ...
  async getHotWallet(@Request() req: any) {
    try {
      ...
      const stellarBalance = await this.stellarService.getAccountInfo(hotWallet.publicKey);
  ...
  async getColdWallet(@Request() req: any) {
    try {
      const coldWallet = await this.walletService.getColdWallet(req.user.userId);
      
      // Get Stellar network balance
      const stellarBalance = await this.stellarService.getAccountInfo(coldWallet.publicKey);
  ...
  async getWalletBalances(@Request() req: any) {
    try {
      const balances = await this.walletService.getWalletBalances(req.user.userId);
