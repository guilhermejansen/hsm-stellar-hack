...
export class KYCService {
  ...
  async submitKYC(
    ...
    try {
      ...
      const kycResult = await this.processKYCWithHSM(
        email,
        phone,
        password,
        kycData,
      ...
      this.logger.log(
        `✅ KYC submitted successfully for user: ${kycResult.userId}`,
      );
    ...
    try {
      ...
      const user = await this.database.user.create({
        data: {
          ...
          // KYC Information
          encryptedPII: hsmPartition.encryptedPII || "", // Svault-encrypted PII
  ...
  private validateKYCData(kycData: KYCSubmission): void {
    ...
    if (
      ...
      kycData.personalInfo.fullName.length < 2
    ) {
      throw new Error(
    ...
    if (
      !kycData.contactInfo.address.street ||
      !kycData.contactInfo.address.city
    ) {
  ...
  async rejectKYC(
    ...
    try {
      ...
      this.logger.log(
        `❌ KYC rejected for user: ${userId} - Reason: ${reason}`,
      );
    } catch (error) {
