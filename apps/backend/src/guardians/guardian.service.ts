...
export class GuardianService {
  ...
  async registerGuardian(registration: GuardianRegistration): Promise<{
    ...
    try {
      this.logger.log(`👥 Registering guardian: ${registration.role} - ${registration.email}`);
      ...
      if (existingGuardian) {
        throw new Error(`Guardian role ${registration.role} is already assigned`);
      }
      ...
      const kycResult = await this.kycService.submitKYC(
        ...
        crypto.randomBytes(32).toString('hex'), // Temporary password
        {
          ...
          documents: {
            idDocument: crypto.createHash('sha256').update(registration.kycData.documentId).digest('hex'),
            proofOfAddress: crypto.createHash('sha256').update(registration.kycData.address).digest('hex'),
            additionalDocs: []
          },
          contactInfo: {
      ...
      this.logger.log(`✅ Guardian registered successfully: ${registration.role} (${guardian.id})`);
   ...
   */
  private async generateTOTPForGuardian(userId: string): Promise<GuardianTOTPSetup> {
    try {
      ...
      const issuer = this.configService.get('TOTP_ISSUER', 'Stellar Custody MVP');
  ...
  async activateGuardian(guardianId: string, totpCode: string): Promise<void> {
    try {
      ...
      this.logger.log(`✅ Guardian activated: ${guardian.role} (${guardianId})`);
    } catch (error) {
      await this.auditService.logGuardianAction(
        guardianId,
        'activation_failed',
  ...
  async updateGuardianStatus(
    ...
    try {
      this.logger.log(`🔄 Updating guardian status: ${guardianId} -> ${isActive ? 'active' : 'inactive'}`);
      ...
      if (!isActive) {
        ...
        if (activeCount < 2) {
          throw new Error('Cannot deactivate guardian: minimum 2 active guardians required');
        }
      }
      ...
      this.logger.log(`✅ Guardian status updated: ${guardian.role} -> ${isActive ? 'active' : 'inactive'}`);
    } catch (error) {
   ...
   */
  async validateGuardianTOTP(guardianId: string, totpCode: string): Promise<boolean> {
    try {
      const guardian = await this.database.guardian.findUnique({
        where: { id: guardianId }
      });
  ...
  async getGuardianStats() {
    try {
      ...
      return {
        ...
        avgApprovalsPerGuardian: Math.round(approvalStats._avg.totalApprovals || 0),
        completionRate: totalGuardians > 0 ? (hsmActivatedGuardians / totalGuardians) * 100 : 0
      };
    } catch (error) {
   ...
   */
  async checkGuardianLimits(guardianId: string, amount: string): Promise<{
    canApprove: boolean;
    reason?: string;
