...
export class WhatsAppService {
  ...
  constructor(
    ...
    this.apiUrl = this.configService.get('WHATSAPP_API_URL', 'https://api.zuckzapgo.com');
    this.token = this.configService.get('WHATSAPP_TOKEN', '!!qYWdJ61zk3i1AvTfXhzE!!');
  ...
  async sendText(phone: string, message: string, metadata?: any): Promise<string> {
    try {
      ...
      const cleanPhone = phone.startsWith('+55') ? phone.substring(3) : phone.replace('+', '');
  ...
  async sendApprovalButton(approval: WhatsAppApprovalButton): Promise<string> {
    try {
      this.logger.log(`📱 Sending approval button to ${approval.guardianRole}: ${approval.guardianPhone}`);
   ...
   */
  async sendSuccessSticker(phone: string, transactionHash: string): Promise<string> {
    try {
      ...
      const stickerResponse = await this.whatsappClient.post('/chat/send/sticker', {
        Phone: cleanPhone,
   ...
   */
  async sendColdWalletAuthRequest(approval: WhatsAppApprovalButton & { 
    isColdWallet: boolean;
    requiresOCRA: boolean;
  }): Promise<string> {
    try {
      this.logger.log(`🔐 Sending Cold Wallet auth request to ${approval.guardianRole}: ${approval.guardianPhone}`);
      ...
      this.logger.log(`✅ Cold Wallet auth request sent to ${approval.guardianRole}`);
      return buttonResponse.data?.id || 'unknown';
    } catch (error) {
      this.logger.error(`❌ Failed to send Cold Wallet auth request:`, error.message);
      throw error;
    }
  }
   ...
   */
  async sendTOTPSetup(phone: string, qrCodeUrl: string, guardianRole: string): Promise<string> {
    try {
   ...
   */
  private mapMessageTypeToNotificationType(type: string): 'APPROVAL_REQUEST' | 'APPROVAL_SUCCESS' | 'TRANSACTION_SUCCESS' | 'SECURITY_ALERT' {
    switch (type) {
      case 'button':
  ...
  async notifyAllGuardians(message: string, metadata?: any): Promise<string[]> {
    try {
      this.logger.log(`📢 Broadcasting to all guardians: ${message.substring(0, 50)}...`);
   ...
   */
  async sendEmergencyAlert(message: string, severity: 'LOW' | 'HIGH' | 'CRITICAL'): Promise<void> {
    try {
