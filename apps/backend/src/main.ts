...
async function bootstrap() {
  ...
  if (mtlsEnabled) {
    app.use((req, res, next) => {
      ...
      if (!isAuthorized) {
        logger.warn(
          `❌ mTLS: Invalid certificate CN: ${clientCert.subject.CN} from ${req.ip}`,
        );
  ...
  logger.log(
    `🌐 Server running on: ${mtlsEnabled ? "https" : "http"}://localhost:${port}`,
  );
  ...
  if (nodeEnv === "development") {
    ...
    logger.log(
      `🗄️  Database: ${configService.get("DATABASE_URL")}:${configService.get("DATABASE_PORT")}`,
    );
    logger.log(
      `🔐 HSM: ${configService.get("HSM_HOST")}:${configService.get("HSM_PORT")}`,
    );
